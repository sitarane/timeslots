...
class ScoresTest < ActiveSupport::TestCase
  ...
  test '#assign_hated_by_all_minus_one' do
    assert ScoreBoard.new(@score_board).assign_hated_by_all_minus_one == { 2 => 1 }
    assert @score_board.length == 3
  end
  test '#assign_most_wanted' do
    assert ScoreBoard.new(@most_wanted).assign_most_wanted == { 1 => 2 }
    assert @most_wanted.length == 1
  end
  test '#assign_most_hated_to_someone_who_wants_it' do
    assert ScoreBoard.new(@score_board).assign_most_hated_to_someone_who_wants_it == { 2 => 1 }
    assert @score_board.length == 3
    assert ScoreBoard.new(@most_hated).assign_most_hated_to_someone_who_wants_it == { 3 => 1 }
    assert @most_hated.length == 2
  end
  test '#assign_at_random' do
    assert ScoreBoard.new(@most_wanted).assign_at_random == { 1 => 1 }
    assert @most_wanted.length == 1
    assert ScoreBoard.new(@most_hated).assign_at_random == { 1 => 1 }
    assert @most_hated.length == 2
    assert ScoreBoard.new(@bad_board).assign_at_random == { 1 => 1 }
